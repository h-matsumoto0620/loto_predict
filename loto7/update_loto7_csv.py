...
# 設定
# ============================================================
DOWNLOAD_URL = "https://loto7.thekyo.jp/data/loto7.csv"
...
def parse_table(table, target_round: int) -> dict | None:
    ...
    num_cells = []
    for t in cell_texts:
        if re.fullmatch(r"0?([1-9]|[12]\d|3[0-7])", t):
    ...
    for t in cell_texts:
        ...
        if m:
            bonus_list.append(int(m.group(1)))

    if len(num_cells) >= 7 and len(bonus_list) >= 2:
        return {
            "round":   target_round,
            "date":    date or "不明",
            "numbers": sorted(num_cells[:7]),
            "bonus1":  bonus_list[0],
            "bonus2":  bonus_list[1],
        }
    return None
            ...
            for table in tables:
                ...
                if data:
                    ...
                    print(f"  [OK] 第{target}回 取得成功: "
                          f"本数字={data['numbers']} "
                          f"ボーナス=[{data['bonus1']}, {data['bonus2']}] ({data['date']})")
    ...
    with open(file_path, "a", encoding="shift_jis", errors="replace", newline="") as f:
        ...
        for rec in sorted(new_records, key=lambda r: r["round"]):
            row = {
                ...
                NUM_COLS[6]:   rec["numbers"][6],
                BONUS_COLS[0]: rec["bonus1"],
                BONUS_COLS[1]: rec["bonus2"],
            }
            writer.writerow(row)
            print(f"  追記: 第{rec['round']}回  {fmt_date(rec['date'])}  "
                  f"本数字={rec['numbers']} "
                  f"ボーナス=[{rec['bonus1']}, {rec['bonus2']}]")
