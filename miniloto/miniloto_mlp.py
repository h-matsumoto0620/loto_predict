...
y_train, y_test = y[train_idx], y[test_idx]   # (n_train, 5), (n_test, 5)

# 5列を1モデルで同時学習（Dense(5)出力）
...
preds_test = model.predict(X_test, verbose=0)   # (n_test, 5)
...
raw_preds = model.predict(next_feat_scaled, verbose=0)[0]   # (5,)
