...
# 設定
# ============================================================
DOWNLOAD_URL = "https://miniloto.thekyo.jp/data/miniloto.csv"
MIZUHO_URL   = "https://www.mizuhobank.co.jp/takarakuji/check/loto/miniloto/index.html"
CSV_FILENAME = "miniloto.csv"
...
def parse_table(table, target_round: int) -> dict | None:
    ...
    num_cells = []
    for t in cell_texts:
        if re.fullmatch(r"0?([1-9]|[12]\d|3[01])", t):
    ...
    if len(num_cells) >= 5 and bonus is not None:
        return {
            "round":   target_round,
            "date":    date or "不明",
            "numbers": sorted(num_cells[:5]),
            "bonus":   bonus,
        }
    return None
